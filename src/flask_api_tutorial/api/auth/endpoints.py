...
class LoginUser(Resource):
    ...
    @auth_ns.response(int(HTTPStatus.UNAUTHORIZED), "Email or password do not match.")
    @auth_ns.response(int(HTTPStatus.BAD_REQUEST), "Validation error.")
    @auth_ns.response(int(HTTPStatus.INTERNAL_SERVER_ERROR), "Internal server error.")
    def post(self):
...
class LogOutUser(Resource):
    ...
    @auth_ns.response(int(HTTPStatus.OK), "Log out successful, token is no longer valid.")
    @auth_ns.response(int(HTTPStatus.BAD_REQUEST), "Validation error.")
...
class GetUser(Resource):
    """Handles HTTP requestrs to URL: /api/v1/auth/user."""
